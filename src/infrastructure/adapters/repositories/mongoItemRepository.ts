...
export class MongoItemRepository implements ItemRepositoryPort {
  async createItem(item: Item): Promise<Item> {
    const itemDoc = new ItemModel({ id: randomUUID(), name: item.name, link: item.link, quantity: item.quantity, place: item.place });
    await itemDoc.save();
    return new Item(itemDoc.id, itemDoc.name, itemDoc.link, itemDoc.quantity, itemDoc.place);
  }

  async getAllItens(): Promise<Item[]> {
    const itemDocs = await ItemModel.find();
    return itemDocs.map(itemDoc => new Item(itemDoc.id, itemDoc.name, itemDoc.link, itemDoc.quantity, itemDoc.place));
  }
  ...
  async updateItem(quantity: number, name: string): Promise<Item> {
    ...
    return new Item(itemDocUpdate?.id, itemDocUpdate?.name || '', itemDocUpdate?.link || '', itemDocUpdate?.quantity || 0, itemDocUpdate?.place || '');
  }
}
